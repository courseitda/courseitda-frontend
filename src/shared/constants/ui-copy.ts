// 화면 카피 상수 - 토스트/검증 메시지와 분리해 변하기 쉬운 안내 문구를 관리
export const UI_COPY = {
  common: {
    brandName: '코스잇다',
    loading: '불러오는 중...',
    retry: '새로고침',
    close: '닫기',
    cancel: '취소',
    delete: '삭제',
    edit: '수정',
    deleting: '삭제 중...',
    search: '검색',
    add: '추가',
    adding: '추가 중...',
    upload: '업로드',
    more: '더보기',
    countSuffix: '개',
    placeCountSuffix: '곳',
    selectedFallback: '선택한',
  },
  system: {
    authTokenRequired: '인증 토큰이 필요합니다.',
    loginRequired: '로그인이 필요합니다.',
    loginRequiredForFeature: '로그인 후 이용할 수 있는 기능입니다.',
    workspaceIdentifierRequired: '워크스페이스 식별자가 필요합니다.',
    notSpecified: '미지정',
  },
  loginRequiredDialog: {
    title: '로그인이 필요해요',
    description: '해당 기능을 이용하시려면 먼저 로그인해주세요.',
    action: '로그인 하기',
    featureDescription: (featureName: string) => `${featureName}을 이용하시려면 먼저 로그인해주세요.`,
  },
  auth: {
    pageTitle: '인증',
    brandTitle: '코스잇다',
    subtitle: '코스 계획을 시작해보세요 ☺️',
    accountCardTitle: '계정',
    accountCardDescription: '로그인하거나 새 계정을 만드세요',
    loginTab: '로그인',
    registerTab: '회원가입',
    loginForm: {
      emailPlaceholder: '이메일',
      passwordPlaceholder: '비밀번호',
      submit: '로그인',
      submitting: '로그인 중...',
    },
    registerForm: {
      nicknamePlaceholder: '닉네임',
      emailPlaceholder: '이메일',
      passwordPlaceholder: '비밀번호',
      confirmPasswordPlaceholder: '비밀번호 확인',
      checkAction: '확인',
      checkingAction: '확인 중...',
      nicknameAvailable: '✓ 사용 가능한 닉네임입니다',
      nicknameDuplicated: '사용중인 닉네임입니다',
      nicknameMinLength: '최소 2자 이상',
      nicknameMaxLength: '최대 20자 이하',
      invalidEmailFormat: '유효하지 않은 이메일 형식입니다',
      emailDuplicated: '이미 사용중인 이메일입니다',
      emailAvailable: '✓ 사용 가능한 이메일입니다',
      passwordMinLength: '최소 6자 이상',
      passwordMaxLength: '최대 20자 이하',
      passwordMismatch: '비밀번호가 일치하지 않습니다.',
      submit: '회원가입',
      submitting: '가입 중...',
    },
    noAccount: '아직 계정이 없나요?',
  },
  notFound: {
    pageTitle: '페이지 오류',
    title: '페이지를 찾을 수 없습니다',
    description: '요청하신 페이지가 존재하지 않거나 이동되었습니다.',
    action: '홈으로 돌아가기',
  },
  myPage: {
    pageTitle: '마이페이지',
    logout: '로그아웃',
  },
  landing: {
    hero: {
      line1: '시작은 간편하게',
      line2: '마무리는 완벽하게',
      brand: '코스잇다',
      suffix: '에서!',
      description: '처음부터 고민하지 말고, 필요한 컬렉션만 골라 간편하게 시작하세요.',
      start: '시작하기',
      moveToWorkspace: '내 워크스페이스로 이동',
      exploreCommunity: '커뮤니티 둘러보기',
    },
    problemsTitle: '이런 고민, 있지 않나요?',
    featuresTitle: '핵심 기능',
    stepsTitle: '이렇게 사용해요',
    problems: [
      {
        title: '분산된 정보',
        description: '지도 즐겨찾기, 메모, 채팅에 흩어진 장소를 한 곳에서 체계적으로 관리하기 어렵습니다.',
      },
      {
        title: '장소 탐색 피로도',
        description: '내 취향에 맞는 코스 정보를 찾기 위해 여러 플랫폼을 오가며 탐색해야 합니다.',
      },
      {
        title: '복잡한 코스 선정 과정',
        description: '장소를 찾은 뒤에도 순서 정리와 동선 확인까지 직접 하려면 시간이 오래 걸립니다.',
      },
    ],
    features: [
      {
        title: '커뮤니티 탐색과 저장',
        description: '다른 사용자가 공유한 컬렉션을 둘러보고, 마음에 드는 컬렉션을 내 보관함에 모을 수 있습니다.',
      },
      {
        title: '커스텀 코스 제작',
        description: '컬렉션 순서를 편집하고 대표 장소를 정해, 완벽한 일정을 만들 수 있습니다.',
      },
      {
        title: '지도 기반 시각화',
        description: '컬렉션별 색상 마커와 대표 장소 경로로 동선을 한눈에 확인할 수 있습니다.',
      },
      {
        title: '공유와 재사용',
        description: '내가 만들어둔 컬렉션을 재사용하거나 공유할 수 있습니다.',
      },
    ],
    steps: [
      {
        number: '01',
        title: '커뮤니티에서 컬렉션을 찾습니다',
        description: '다른 사용자가 공유한 컬렉션을 둘러보고, 원하는 컬렉션을 고릅니다.',
      },
      {
        number: '02',
        title: '워크스페이스로 가져옵니다',
        description: '고른 컬렉션을 내 워크스페이스 일정으로 불러와 바로 사용합니다.',
      },
      {
        number: '03',
        title: '필요한 만큼만 커스텀합니다',
        description: '컬렉션 순서를 편집하고 대표 장소를 설정해 일정 흐름을 정리합니다.',
      },
      {
        number: '04',
        title: '완성한 컬렉션을 다시 공유합니다',
        description: '내가 다듬은 컬렉션을 공유해 다른 사용자도 쉽게 코스를 시작할 수 있습니다.',
      },
    ],
  },
  myWorkspace: {
    pageTitle: '내 워크스페이스',
    createAction: '새 워크스페이스',
    titleRequired: '워크스페이스 제목을 입력해주세요.',
    updatedPrefix: '업데이트',
    moreActionAriaLabel: '워크스페이스 더보기',
    empty: {
      title: '워크스페이스가 없습니다.',
      description: '지금 추가해보세요!',
    },
    deleteDialog: {
      title: '워크스페이스 삭제',
      description: (title: string) => `"${title}" 워크스페이스를 정말 삭제하시겠습니까?`,
      warning: '이 작업은 되돌릴 수 없으며, 모든 컬렉션과 장소 정보가 함께 삭제됩니다.',
    },
  },
  workspaceDialog: {
    create: {
      title: '새 워크스페이스',
      fieldLabel: '제목',
      placeholder: '예: 홍대 데이트 코스',
      cancel: '취소',
      submit: '생성',
      submitting: '생성 중...',
    },
    edit: {
      title: '워크스페이스명 수정',
      fieldLabel: '제목',
      placeholder: '예: 홍대 데이트 코스',
      cancel: '취소',
      submit: '확인',
      submitting: '수정 중...',
    },
  },
  myCategory: {
    pageTitle: '내 컬렉션',
    createAction: '새 컬렉션',
    archiveTab: '보관',
    favoriteTab: '찜',
    pageTitleShort: '컬렉션 상세',
    nameRequired: '컬렉션 이름을 입력해주세요.',
    nameMaxLength: '컬렉션 이름은 10자를 초과할 수 없습니다.',
    placeSearchKeywordRequired: '검색어를 입력해주세요.',
    placeSearchNoResult: '검색 결과가 없습니다.',
    placeAlreadyAdded: '이미 추가된 장소입니다.',
    atLeastOnePlace: '장소를 1개 이상 추가해주세요.',
    moreActionAriaLabel: '컬렉션 더보기',
    updatedPrefix: '업데이트',
    deleteAction: '삭제하기',
    empty: {
      title: '컬렉션이 없습니다.',
      description: '지금 추가해보세요!',
    },
    favoriteEmpty: {
      title: '찜한 컬렉션이 없습니다.',
      description: '나중에 찜한 항목이 여기에 표시됩니다.',
    },
    detailDialog: {
      placeListTitle: '장소 목록',
      noPlacesInDetail: '표시할 장소가 없습니다.',
      editingPlaceListTitle: '추가된 장소',
      renameAriaLabel: '컬렉션 이름 변경',
      editAction: '수정하기',
      addCompleted: '추가됨',
    },
    editorDialog: {
      createTitle: '새 컬렉션 추가',
      editTitle: '컬렉션명 수정',
      nameLabel: '컬렉션 이름',
      namePlaceholder: '예: 맛집 투어',
      selectedPlacesLabel: '추가된 장소',
      proceedToPlaceAction: '장소 추가하기',
      placeSearchLabel: '장소 검색',
      placeSearchPlaceholder: '장소 이름이나 주소 검색',
      searchAction: '검색',
      searching: '검색 중...',
      noPlacesSelected: '선택한 장소가 없습니다.',
      removePlaceAriaLabel: '장소 제거',
      addPlaceAction: '추가',
      cancel: '취소',
      create: '생성',
      creating: '생성 중...',
      save: '저장',
      saving: '저장 중...',
      edit: '수정',
      editing: '수정 중...',
    },
    leaveConfirm: {
      title: '저장되지 않은 내용이 있어요',
      description: '아직 저장하지 않은 컬렉션 내용이 있습니다. 이동하면 작성 중인 내용이 사라집니다. 이동하시겠어요?',
      stay: '계속 작성',
      leave: '이동하기',
    },
    deleteDialog: {
      title: '컬렉션 삭제',
      description: (title: string) => `"${title}" 컬렉션을 정말 삭제하시겠습니까?`,
      warning: '이 작업은 되돌릴 수 없으며, 컬렉션에 포함된 장소 정보도 함께 삭제됩니다.',
    },
  },
  categoryDialog: {
    add: {
      title: '컬렉션 추가',
      colorLabel: '색상 선택',
      suggestedLabel: '추천 컬렉션',
      manualInputLabel: '직접 입력',
      manualInputPlaceholder: '컬렉션 이름',
      cancel: '취소',
      submit: '추가',
      submitting: '추가 중...',
    },
    edit: {
      title: '컬렉션 수정',
      colorLabel: '색상 선택',
      nameLabel: '컬렉션 이름',
      namePlaceholder: '컬렉션 이름',
      cancel: '취소',
      submit: '확인',
      submitting: '수정 중...',
    },
    suggestedCategories: ['점심', '카페', '산책', '쇼핑', '저녁'],
    paletteToggleAriaLabel: '색상 팔레트 변경',
    colorSelectAriaLabel: (color: string) => `색상 ${color} 선택`,
  },
  myPosts: {
    pageTitle: '내 게시물',
    sectionTitle: '업로드한 컬렉션',
    uploadAction: '업로드하기',
    moreActionAriaLabel: '게시물 더보기',
    empty: {
      title: '업로드한 컬렉션이 없습니다.',
      description: '내 컬렉션을 업로드해보세요.',
    },
    deleteDialog: {
      title: '게시물을 삭제할까요?',
      description: (title: string) => `"${title}" 게시물을 삭제합니다.`,
      warning: '삭제 후에는 복구할 수 없습니다.',
    },
  },
  sharedCategoryDetail: {
    placeListTitle: '장소 목록',
  },
  workspaceDetail: {
    notFound: '워크스페이스를 찾을 수 없습니다.',
    createAction: '새 워크스페이스',
    mapNotReady: '네이버 지도 설정이 완료되지 않았습니다. 관리자에게 문의해주세요.',
    renameAriaLabel: '워크스페이스 이름 변경',
    categoryPanelResizeAriaLabel: '컬렉션 패널 높이 조절',
  },
  community: {
    pageTitle: '커뮤니티',
    recommendedTitle: '코스잇다 추천 컬렉션',
    boardTitle: '컬렉션 게시판',
    boardPageTitle: '컬렉션 게시판',
    uploadFabLabel: '업로드',
    searchPlaceholder: '장소 컬렉션을 찾아보세요',
    searchActionAriaLabel: '검색',
    emptySearchResult: '검색 결과가 없습니다.',
    boardMoreAction: '더보기',
  },
  toaster: {
    close: '닫기',
  },
  categoryList: {
    title: '컬렉션',
    addAction: '추가',
    importAction: '불러오기',
    createAction: '생성하기',
    loadFailed: '컬렉션을 불러오지 못했습니다. 잠시 후 다시 시도해주세요.',
    emptyDescription: '컬렉션을 추가해보세요',
    reorderToggle: '순서 조정',
    confirmAction: '확인',
    moveUpAriaLabel: '위로 이동',
    moveDownAriaLabel: '아래로 이동',
    viewModeAriaLabel: '보기 모드',
    editModeAriaLabel: '편집 모드',
  },
  communitySearch: {
    sectionTitle: '추천 검색어',
    regionTitle: '🗺️ 지역',
    foodTitle: '🍽️ 식사',
    activityTitle: '🎯 활동',
    regionKeywords: [
      { value: '성수', label: '성수' },
      { value: '연남', label: '연남' },
      { value: '잠실', label: '잠실' },
      { value: '강남', label: '강남' },
    ],
    foodKeywords: [
      { value: '점심', label: '점심' },
      { value: '저녁', label: '저녁' },
      { value: '카페', label: '카페' },
      { value: '브런치', label: '브런치' },
      { value: '디저트', label: '디저트' },
    ],
    activityKeywords: [
      { value: '전시', label: '전시' },
      { value: '영화', label: '영화' },
      { value: '공방', label: '공방' },
      { value: '보드게임', label: '보드게임' },
      { value: '방탈출', label: '방탈출' },
    ],
  },
  searchResult: {
    pageTitle: '검색 결과',
    sectionTitle: '검색 결과',
  },
  userMenu: {
    sheetTitle: '메뉴',
    triggerAriaLabel: '사용자 메뉴',
    storageSection: '보관함',
    communitySection: '커뮤니티',
    accountSection: '계정',
    myWorkspace: '내 워크스페이스',
    myCategory: '내 컬렉션',
    communityExplore: '커뮤니티 둘러보기',
    myPosts: '내 공유 목록',
    myPage: '마이페이지',
    login: '로그인',
    logout: '로그아웃',
  },
  navigationDrawer: {
    sheetTitle: '메뉴',
    workspaceSection: '워크스페이스',
    createWorkspace: '새 워크스페이스',
    openMenuAriaLabel: '메뉴 열기',
    renameWorkspace: '이름 바꾸기',
  },
  uploadCategoryDialog: {
    title: '컬렉션 업로드',
    noticeBadge: '안내',
    description: '내 컬렉션 중 커뮤니티에 공유할 항목을 선택하세요.',
    empty: {
      title: '업로드 할 컬렉션이 없습니다.',
      description: '내 컬렉션에서 먼저 만들어보세요.',
      action: '내 컬렉션 만들러 가기',
    },
  },
  importCategoryDialog: {
    title: '컬렉션 불러오기',
    savedTab: '내 컬렉션',
    savedEmpty: '내 컬렉션이 없습니다.',
    favoriteTab: '찜한 컬렉션',
    favoriteEmpty: '찜한 컬렉션이 없습니다.',
  },
  placeSearchDialog: {
    title: '장소 검색',
    description: '장소를 검색하고 추가하세요',
    placeholder: '장소 이름이나 주소 검색',
    searchAction: '검색',
    searching: '검색 중...',
    addAction: '추가',
    addingAction: '추가 중...',
    keywordRequired: '검색어를 입력해주세요.',
    noResult: '검색 결과가 없습니다.',
  },
  place: {
    linkMissing: '장소 링크가 없습니다.',
    openSearchAction: '장소 검색',
    emptyPrompt: '장소를 추가해보세요',
    menuOpenAriaLabel: '장소 메뉴 열기',
    openInMapAction: '지도 바로가기',
    setRepresentativeAction: '대표 장소 지정',
    unsetRepresentativeAction: '대표 장소 해제',
    deleteAction: '삭제',
  },
  map: {
    loading: '지도 로딩 중...',
    locateMeAriaLabel: '내 위치로 이동',
    browserLocationUnsupported: '이 브라우저에서는 위치 정보를 지원하지 않습니다.',
    mapNotReady: '지도가 아직 준비되지 않았습니다.',
    locationPermissionDenied: '위치 권한이 거부되었습니다. 브라우저 설정을 확인해주세요.',
    locationFetchFailed: '현재 위치를 가져오지 못했습니다. 다시 시도해주세요.',
  },
  pageHeader: {
    backAriaLabel: '뒤로가기',
    homeAriaLabel: '홈으로 이동',
  },
  sidebar: {
    toggle: '사이드바 토글',
  },
  relativeTime: {
    invalid: '-',
    justNow: '방금 전',
    minutesAgo: (value: number) => `${value}분 전`,
    hoursAgo: (value: number) => `${value}시간 전`,
    daysAgo: (value: number) => `${value}일 전`,
    weeksAgo: (value: number) => `${value}주 전`,
    monthsAgo: (value: number) => `${value}개월 전`,
    yearsAgo: (value: number) => `${value}년 전`,
  },
  spinner: {
    statusAriaLabel: '로딩 중',
    srOnly: '로딩 중...',
    defaultMessage: '로딩 중...',
    routeMessages: [
      '지도를 불러오는 중입니다...',
      '마커를 찍는 중입니다...',
      '경로를 잇는 중입니다...',
      '최적 경로를 계산하는 중입니다...',
    ],
  },
} as const;
