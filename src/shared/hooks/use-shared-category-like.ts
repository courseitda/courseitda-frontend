...
const updateCategoryLikeState = (
  category: SharedSavedCategory,
  nextLiked: boolean,
): SharedSavedCategory => ({
  ...category,
  liked: nextLiked,
  likeCount: Math.max(0, (category.likeCount ?? 0) + (nextLiked ? 1 : -1)),
});

// UserRequest: 공유 컬렉션 찜 토글 로직을 공통 훅으로 분리
  ...
  const toggleLikeMutation = useMutation({
    ...
    onMutate: async (params) => {
      ...
      queryClient.setQueryData<SharedSavedCategory[]>(queryKey, (old) =>
        (old ?? []).map((category) =>
          category.id === params.sharedCategoryId
            ? updateCategoryLikeState(category, params.nextLiked)
            : category,
        ),
      );
      ...
      if (setSelectedCategory) {
        setSelectedCategory((previousSelected) =>
          previousSelected && previousSelected.id === params.sharedCategoryId
            ? updateCategoryLikeState(previousSelected, params.nextLiked)
            : previousSelected,
        );
      }
    ...
    onSettled: () => {
      queryClient.invalidateQueries({ queryKey: ['community', 'shared-categories'] });
    },
  });
